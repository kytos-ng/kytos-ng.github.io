...
- Tune default value for API thread pool size and its concurrency limit
...
Kytos-ng core infrastructure UI and NApps UI have been upgraded to Vue.js 3 in compatibility mode (with the `@vue/compat` package). Improvements have also been included for the production build to have more informative and detailed messages. This has been a pivotal upgrade due to Vue 2 reaching EOL (end of life) date support.
