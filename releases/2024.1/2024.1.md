...
The `api_concurrency_limit` on `kytos.conf` which defaults to the same value as the `api` thread pool size on `thread_pool_max_workers`, now has a larger default value. If you are upgrading from prior versions, it is recommended that you regenerate `kytos.conf` again or just set the individual new valuable. If either of these values are too low for the scalability of network resources that one is using, log warning messages will be emitted.
...
- [126 pull requests were merged](https://github.com/search?q=org%3Akytos-ng+is%3Apr+is%3Aclosed+merged%3A2024-03-01..2024-08-31&type=Issues)
- [76 issues were closed](https://github.com/search?q=org%3Akytos-ng+is%3Aissue+is%3Aclosed+closed%3A2024-03-01..2024-08-31+label%3A2024.1&type=issues)
