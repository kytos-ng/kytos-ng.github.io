...
- Changed `telemetry_int` proxy ports to be optional for inter EVCs
- Removed `mef_eline` QinQ encapsulation for vlan translation cases
- Added support for virtual NoviFlow switches on e2e tests
- Initial P4 switches support with pyp4ofagent
- Fixed `kytos_stats` interface utilization charts
...
### 1. Changed `telemetry_int` proxy ports to be optional for inter EVCs

Proxy ports are now optional for inter EVCs. A proxy port is used if it has `proxy_port` metadata configured on UNIs. You also use EVC metadata `proxy_port_enabled` to control whether or not that given EVC is supposed to use a proxy port, this configuration knob is useful for additional fine control, which is useful when you either not want to use a proxy port on an EVC on UNIs that have proxy port configuration or when you want to explicitly enforce the expected proxy port enable configuration for a given EVC.

`telemetry_int` now also has a consistency check which will verify if ``telemetry_int`` flows for a given EVC are installed as expected, if they aren't, they will be categorized as either missing or alien (extra) flows, which the consistency check can also fix specifically installing the missing ones or removing the extra ones. On version `2025.2` this consistency check doesn't run automatically yet, so if you are using it in a production environment, you are expected to make periodic request to the consistency check endpoint to trigger it. In a future version, the consistency check will run automatically by `telemetry_int`.

### 2. Removed ``mef_eline`` QinQ encapsulation for vlan translation cases
...
### 3. Added suport for virtual NoviFlow switches on e2e tests

Kytos-ng e2e test suites, composed of hundreds of test cases, now also support targeting virtual NoviFlow switches in addition to OvS. A mininet switch wrapper has been developed to interface with virtual NoviFlow allowing it to be used in a mininet topology, this wrapper also has been implemented to seamless support it as if it were an OvS switch compatible with the other mininet commands that are already used in tests assertions. For more information check out [this documentation](https://github.com/kytos-ng/kytos-end-to-end-tests/blob/master/misc/README-NoviSwi.md).

### 4. Initial P4 switches support with pyp4ofagent

Kytos-ng team has been researching solution to support additional southbound protocols. Strategically, at the moment, AmLight has researched and decided to develop a local OpenFlow agent that bridges and exposes a P4 switch for Kytos-ng as a OpenFlow switch. This solution and architecture also open new options to locally implement a feature in the switches, while also minimizing development effort to support P4 switches. This unlocks P4 switches to be used with all the functionality the existing OpenFlow NApps already support. Also, `pyof` (python-openflow) has been fixed to support table features properties and other related serialization and deserialization issues.

### 5. Fixed `kytos_stats` interface utilization charts
...
### 8. Move Link objects to Kytos-ng core
...
- [`000_liveness_deleted.py`](https://github.com/kytos-ng/of_lldp/blob/master/scripts/db/2025.2.0/000_liveness_deleted.py) can be used to list and delete old non existent interfaces which had liveness enabled, but hadn't been fully removed from DB documents. If the `list` command didn't return any document then the DB collection doesn't have any left over to be deleted.
