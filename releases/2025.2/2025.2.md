...
- Remove `mef_eline` QinQ encapsulation for vlan translation cases
...
- Augment interface tag pools to support differentiating between UNI and NNIs
...
`telemetry_int` now also has a consistency check which will verify if ``telemetry_int`` flows for a given EVC are installed as expected. `telemetry_int` also exposes consistency check actions to fix these inconsistent flows via a REST endpoint. On version `2025.2`, this consistency check doesn't run automatically yet. If you are using `telemetry_int` in a production environment, it's highly recommended to have a periodic script making request to this consistency check endpoint.

### 2. Remove ``mef_eline`` QinQ encapsulation for vlan translation cases
...
### 6. Augment interface tag pools to support differentiating between UNI and NNIs
...
**TODO:** this will be delivered as a patch on 2025.2. Remove this paragraph when it's ready.
