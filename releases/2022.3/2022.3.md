...
NoviFlow switches are now supported on [kytos-end-to-end-tests](https://github.com/amlight/kytos-end-to-end-tests/) as a target on pre-defined [testing topologies](https://github.com/amlight/kytos-end-to-end-tests/blob/feature/noviswitch/tests/helpers.py). Currently, NoviFlow switches are managed via SSH using `paramiko` and this integration was designed to be seamless with mininet, NoviFlow switches are handled as a mininet `Switch` so you can reuse or write new end-to-end tests.
...
`flow_stats` has had its API reviewed and augmented consolidating its v1 API. `flow_stats` has simplified its code and responsibilities.
